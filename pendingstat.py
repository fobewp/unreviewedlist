...
queries = [
	...
	"SELECT CONCAT(REPLACE(ROUND(100*(((SELECT COUNT(*) FROM flaggedpages INNER JOIN page ON page_id = fp_page_id WHERE page_namespace = 0)-(SELECT COUNT(*) FROM flaggedpage_pending INNER JOIN page ON page_id = fpp_page_id WHERE page_namespace = 0))/(SELECT COUNT(*) FROM page WHERE page_namespace = 0)),2),'.',','),'%')",
